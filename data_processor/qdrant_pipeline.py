...
from flexit_llm.utility.qdrant_vector_store import (
    QdrantConfig,
...
class QdrantPipeline:
    ...
    def generate_embeddings(self, batch_size: int = 20) -> Dict[str, Any]:
        ...
        # Initialize cache loader and load chunks from cache
        logger.info("Loading chunks from cache...")
        cache_stats = self.cache_loader.initialize()
        logger.info(f"Cache loaded: {cache_stats['chunks_loaded']} chunks available")

        # Collect chunks and their content from cache
        chunks_to_process = []
        chunk_metadata = {}

        # Access chunks from cache data
        cache_data = self.cache_loader._cache_data
